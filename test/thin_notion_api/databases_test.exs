defmodule ThinNotionApi.DatabasesTest do
  ...
  test "PATCH update_database!" do
    use_cassette "patch_update_database", match_requests_on: [:request_body] do
      {:ok, response} = Databases.update_database!("a4ef92b2a7984bae82114817678cd2f4", "Super New title")
      assert Map.keys(response) == ["cover", "created_time", "icon", "id", "last_edited_time", "object", "parent", "properties", "title", "url"]
      assert NotionTestHelpers.get_in_undasherized(response, ["parent", "page_id"]) == "9b4a624d5a18482ab2187e54166edda7"
      assert get_in(response, ["title"]) |> Enum.at(0) |> get_in(["plain_text"]) == "Super New title"

      {:ok, response} = Databases.update_database!("a4ef92b2a7984bae82114817678cd2f4", "New Database")
      assert get_in(response, ["title"]) |> Enum.at(0) |> get_in(["plain_text"]) == "New Database"
    end
  end
end
